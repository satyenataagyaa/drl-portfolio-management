"""
Use DDPG to train a sector trader based on a window of history price
"""
...
from environment.portfolio import LsPortfolioEnv
from utils.sector_data import read_sector_history, normalize
...
def get_model_path(window_length, predictor_type, use_batch_norm):
    ...
    return 'weights/ls_sector/{}/window_{}/{}/checkpoint.ckpt'.format(predictor_type, window_length, batch_norm_str)
...
def get_result_path(window_length, predictor_type, use_batch_norm):
    if use_batch_norm:
        ...
        batch_norm_str = 'no_batch_norm'
    return 'results/ls_sector/{}/window_{}/{}/'.format(predictor_type, window_length, batch_norm_str)
...
def sector_predictor(inputs, predictor_type, use_batch_norm):
    ...
    if predictor_type == 'cnn':
        ...
        num_sectors = inputs.get_shape()[1]
        hidden_dim = 32
        ...
        net = tflearn.reshape(net, new_shape=[-1, num_sectors, hidden_dim])
        if DEBUG:
...
class StockActor(ActorNetwork):
    ...
    def create_actor_network(self):
        ...
        net = sector_predictor(inputs, self.predictor_type, self.use_batch_norm)
...
def obs_normalizer(observation):
    ...
    observation = observation[:, :, 3:4] / observation[:, :, 2:3]
...
if __name__ == '__main__':
    ...
    with tf.variable_scope(variable_scope):
        ...
        ddpg_model = DDPG(env, sess, actor, critic, actor_noise, obs_normalizer=obs_normalizer,
                          config_file='config/ls_sector.json', model_save_path=model_save_path,

...
#                'MAR', 'REGN', 'SBUX']
target_list = ['XLK', 'XLV', 'XLI', 'XLU', 'XLF', 'XLP', 'XLB', 'XLY', 'XLE']

# target_list_2 = ['FOX', 'FISV', 'EXPE', 'FAST', 'ESRX', 'DLTR', 'CTSH', 'CSCO', 'QCOM', 'PCLN', 'CELG',
#                  'AMGN', 'WFM', 'WDC', 'NVDA', 'STX']
target_list_2 = ['XLK', 'XLV', 'XLI', 'XLU', 'XLF', 'XLP', 'XLB', 'XLY', 'XLE']
...
def create_dataset(filepath):
    """ create the raw dataset from all_sector_5yr.csv. The data is Open,High,Low,Close,Volume

    Args:
        path: path of all_sectors_5yr.csv

    Returns:
        history: numpy array of size (N, number_day, 5),
        abbreviation: a list of sector abbreviation where index map to name
    ...
    with open(filepath, 'r') as csvfile:
        ...
        for row in reader:
            row.pop(6)  # We have no use for adj_close
            if row[6] in exclude_set:
                continue
            if row[6] != current_sector:
                current_sector_index += 1
                # initialize
                ...
                if (date - start_datetime).days != 0:
                    ...
                    continue
                assert (date - start_datetime).days == 0, 'Start date is not 2015-01-02'
                try:
                    if row[5] == '':
                        row[5] = 0
                    ...
                    print(row[6])
                    assert False
                history[current_sector_index][current_date_index] = data
                previous_day_data = data

                current_sector = row[6]
                abbreviation.append(current_sector)
            else:
                date = datetime.datetime.strptime(row[0], date_format)
                # missing date, loop to the date difference is 0
                while (date - current_date).days != 0:
                    history[current_sector_index][current_date_index] = previous_day_data.copy()
...
def write_to_h5py(history, abbreviation, filepath='datasets/sectors_history.h5'):
...
def create_target_dataset(target_list, filepath='datasets/sectors_history_target.h5'):
    """ Create 9 sector history datasets
    ...
    """
    history_all, abbreviation_all = read_sector_history()
    history = None
...
def read_sector_history(filepath='datasets/sectors_history.h5'):
...
if __name__ == '__main__':
    ...
    create_dataset(filepath='datasets/all_sectors_5yr.csv')
